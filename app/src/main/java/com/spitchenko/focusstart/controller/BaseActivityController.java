...
public abstract class BaseActivityController {
    private final static String BASE_ACTIVITY = "android.content.SharedPreferences" +
            ".BaseActivityController";
    private final static String LOCAL_THEME = BASE_ACTIVITY + ".localTheme";

    protected abstract void saveLocalThemeIdToPrefs(final int themeId);

    protected int readThemeIdOrNullFromPrefs(final Context context) {
    ...
    private String getThemeName(final String[] input) {
    ...
    protected void setThemeFromPrefs(final Context context) {
